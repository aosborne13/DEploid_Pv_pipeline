...
def main(args):
    deploid_path = args.deploid_path
    ...
    run_cmd(f"bash {os.path.join(deploid_path, 'extract_DEploid.sh')}")
    run_cmd(f"Rscript {os.path.join(deploid_path, 'scripts/calculate_PLAF_DEploid.R')} DEploid_input/merged.bi.filt.GT.miss0.4.snps.GT.txt.gz DEploid_input/merged.bi.filt.GT.miss0.4.snps.AD.0.txt.gz DEploid_input/merged.bi.filt.GT.miss0.4.snps.AD.1.txt.gz")
    run_cmd(f"bash {os.path.join(deploid_path, 'subset_sites.sh')}")
    run_cmd(f"bash {os.path.join(deploid_path, 'extract_sites.sh')}")
    run_cmd("mkdir run_DEploid")
    run_cmd(f"bash {os.path.join(deploid_path, 'dataExplore_sites.sh')}")
    run_cmd(f"bash {os.path.join(deploid_path, 'run_DEploid_sites.sh')}")
    run_cmd(f"bash {os.path.join(deploid_path, 'compile_props_DEploid_sites.sh')}")
    run_cmd(f"Rscript {os.path.join(deploid_path, 'scripts/compare_clonality_classification.R')}")
    run_cmd("mkdir BEST")
    run_cmd(f"Rscript {os.path.join(deploid_path, 'scripts/resolve_mixed_sites.R')}")
    run_cmd(f"Rscript {os.path.join(deploid_path, 'scripts/create_BEST_reference_panel.R')}")
    run_cmd(f"bash {os.path.join(deploid_path, 'run_DEploid_BEST_sites.sh')}")
    run_cmd(f"Rscript {os.path.join(deploid_path, 'scripts/GT2hmmIBD_BEST_sites.R')}")
...
parser.add_argument('--deploid_path', type=str, help='Path to the DEploid_Pv_pipeline installation directory')
